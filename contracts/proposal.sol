...
contract ProposalContract {
    ...
    function isSupermajority() private view returns(bool) {
    Proposal storage proposal = proposal_history[counter];
    ...
    // Check if at least 75% of total votes are in favor for supermajority
    if (totalVotes > 0 && (approve * 100 / totalVotes) >= 75) {
        return true; // Supermajority achieved
    } else {
        return false; // Supermajority not achieved
    }
}
